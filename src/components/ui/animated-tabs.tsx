...
return (
  <div className={cn("w-full max-w-lg flex flex-col gap-y-1", className)}>
    <div className="flex gap-2 flex-wrap  bg-gradient-to-r from-primary to-secondary backdrop-blur-sm rounded-2xl sticky top-0 py-2 px-4">
      {tabs.map((tab) => (
        <button
          key={tab.id}
          onClick={() => setActiveTab(tab.id)}
          className={cn(
            "relative px-3 py-1.5 text-lg font-semibold rounded-lg text-black outline-none transition-colors"
          )}
        >
          {activeTab === tab.id && (
            <motion.div
          ...
          <span className={`relative z-10 ${activeTab === tab.id ? 'text-white' : 'text-black'}`}>{tab.label}</span>
        </button>
      ))}
    </div>

    <div className="py-8 text-white min-h-[80vh] h-full">
      {tabs.map(
        (tab) =>
          activeTab === tab.id && (
            <motion.div
